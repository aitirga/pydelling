...
if __name__ == "__main__":
    """This script needs to be run inside the folder containing the PFLOTRAN simulation results"""
    argv = sys.argv
    if len(argv) > 1:
        # You can specify a custom constant dt
        post_processing_engine = PostProcessingEngine(dt=float(sys.argv[1]))
    else:
        post_processing_engine = PostProcessingEngine()
    post_processing_engine.run()
